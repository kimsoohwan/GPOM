...
namespace GPOM{
	...
	public:
							 ...
							 bool										fVarianceVector = true)
		{
			...
			// Ks, Kss
			MatrixConstPtr pKs		= m_CovFunc.Ks(pXs, pCovLogHyp); // nxm
			...
			pMu->noalias() = *(m_MeanFunc.Ms(pXs, pMeanLogHyp)) + (pKs->transpose()) * (*pAlpha);
																		...
																		const int								calculationMode = 0)
		{
			...
			//std::cout << "y - m = " << std::endl << *pY_M << std::endl << std::endl;
			...
			if(calculationMode >= 0)
			{
				...
				//std::cout << "pMeanLogHyp = " << std::endl << *pMeanLogHyp << std::endl << std::endl;
				//std::cout << "pCovLogHyp = " << std::endl << *pCovLogHyp << std::endl << std::endl;
				//std::cout << "pLikCovLogHyp = " << std::endl << *pLikCovLogHyp << std::endl << std::endl;
			...
			if(calculationMode <= 0)
			{
				...
				{
					(*pDnlZ)(j++) = m_MeanFunc(pMeanLogHyp, i)->dot(*pAlpha);
				...
				{
					//(*pDnlZ)(j++) = ((Scalar) 0.5f) * (Q * (m_CovFunc(pCovLogHyp, i)->selfadjointView<Eigen::Upper>())).trace(); // [CAUTION] K: upper triangular matrix
					(*pDnlZ)(j++) = ((Scalar) 0.5f) * Q.cwiseProduct(*(m_CovFunc(pCovLogHyp, i))).sum();
					//std::cout << "DnlZ[ " << j-1 << " ] =  " << (*pDnlZ)(j-1) << std::endl;
				}
				...
				{
					//(*pDnlZ)(j++) = (Scalar) 0.5f * (Q * (*m_LikFunc(pLikCovLogHyp, i))).trace();
					//(*pDnlZ)(j++) = ((Scalar) 0.5f) * Q.cwiseProduct(*(m_LikFunc(pLikCovLogHyp, i))).sum(); // [CAUTION] K: upper triangular matrix
					//(*pDnlZ)(j++) = ((Scalar) 0.5f) * (Q.array() * Matrix(m_LikFunc(pLikCovLogHyp, i)->asDiagonal()).array()).sum(); // [CAUTION] K: upper triangular matrix
					(*pDnlZ)(j++) = ((Scalar) 0.5f) * Q.cwiseProduct(Matrix(m_LikFunc(pLikCovLogHyp, i)->asDiagonal())).sum(); // [CAUTION] K: upper triangular matrix
					//std::cout << "DnlZ[ " << j-1 << " ] =  " << (*pDnlZ)(j-1) << std::endl;
				}
			}
		}
	...
	protected:
													...
													VectorPtr							&pAlpha)					// [output] alpha = inv(Kn) * (y-m)
		{
			...
			// K 
			MatrixPtr pKn = m_CovFunc(pCovLogHyp); // [CAUTION] K: upper triangular matrix
			//std::cout << "K = " << std::endl << *pKn << std::endl << std::endl;

			// D = sn2*I = D^(1/2) * D^(1/2)
			//MatrixPtr pD = m_LikFunc(pLikCovLogHyp);
			//VectorPtr pD = m_LikFunc(pLikCovLogHyp);
			pInvSqrtD = m_LikFunc(pLikCovLogHyp);	// sW
			//std::cout << "D = " << std::endl << *pInvSqrtD << std::endl << std::endl;
			...
			// y - m
			(*pY_M).noalias() = (*m_pY) - (*(m_MeanFunc(pMeanLogHyp)));
			//std::cout << "y - m = " << std::endl << *pY_M << std::endl << std::endl;
			...
			//std::cout << "D^(-1/2) * (y - m) = " << std::endl << pInvSqrtD->asDiagonal() * (*pY_M) << std::endl << std::endl;
			//std::cout << "L.solve(D^(-1/2) * (y - m)) = " << std::endl << pL->solve(pInvSqrtD->asDiagonal() * (*pY_M)) << std::endl << std::endl;
			//std::cout << "alpha = " << std::endl << *pAlpha << std::endl << std::endl;
		}
